...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _login() {
    ...
    _performLogin(email: email, password: password);
  }

  Future<void> _performLogin({required String email, required String password}) async {
    try {
      final cred = await FirebaseService.instance.signIn(email: email, password: password);
      final user = cred.user;
      if (user != null) {
        AppStore().currentEmail = user.email ?? email;
        AppStore().currentUser = (user.displayName != null && user.displayName!.isNotEmpty)
            ? user.displayName!
            : email.split('@').first;
        AppStore().currentUid = user.uid;

        // attempt to load profile from Realtime DB and apply if present
        final profile = await FirebaseService.instance.getUserProfile(user.uid);
        if (profile != null) {
          if (profile['name'] != null) AppStore().currentUser = profile['name'];
          if (profile['email'] != null) AppStore().currentEmail = profile['email'];
        }
